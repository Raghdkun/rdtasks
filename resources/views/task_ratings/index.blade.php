                    ...
                    <div class="card-body">
                        ...
                        <div class="filter-section">
                            ...
                            <div class="quick-filters">
                                <button type="button" class="btn btn-outline-primary quick-filter-btn" id="thisMonthBtn">
                                    <i class="fas fa-calendar-alt"></i> This Month
                                </button>
                                <button type="button" class="btn btn-outline-secondary quick-filter-btn" id="clearAllBtn">
                                    <i class="fas fa-times"></i> Clear All
                                </button>
                            </div>

                            <form id="filterForm" method="GET" action="{{ route('task-ratings.index') }}">
                                <div class="filter-row">
                                    ...
                                    <div class="filter-group">
                                        ...
                                        <select class="form-control" id="rating_status" name="rating_status">
                                            ...
                                            <option value="rated"
                                                {{ request('rating_status') == 'rated' ? 'selected' : '' }}>Rated</option>
                                            <option value="pending"
                                                {{ request('rating_status') == 'pending' ? 'selected' : '' }}>Pending
                                            </option>
                                            <option value="edited"
                                                {{ request('rating_status') == 'edited' ? 'selected' : '' }}>Edited</option>
                                        </select>
                                    </div>
                                ...
                                <tbody>
                                    @forelse($completedTasks as $task)
                                        <tr>
                                            ...
                                            <td>{{ $task->completed_on ? $task->completed_on->format('M d, Y') : 'N/A' }}
                                            </td>
                                            <td>
                                                @if ($task->rating)
                                            ...
                                            <td>
                                                @if ($task->rating)
                                                    <a href="{{ route('task-ratings.show', $task) }}"
                                                        class="btn btn-sm btn-info">
                                                        <i class="fas fa-eye"></i> View
                                                    </a>
                                                    <a href="{{ route('task-ratings.edit', $task) }}"
                                                        class="btn btn-sm btn-warning">
                                                        <i class="fas fa-edit"></i> Edit
                                                    </a>
                                                @else
                                                    <a href="{{ route('task-ratings.create', $task) }}"
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#exportCsvBtn').on('click', function() {
                ...
                const ratingStatus = $('#rating_status').val();

                if (taskName) params.append('task_name', taskName);
                if (assigneeName) params.append('assignee_name', assigneeName);
                if (dateFrom) params.append('date_from', dateFrom);
                if (dateTo) params.append('date_to', dateTo);
                if (ratingStatus) params.append('rating_status', ratingStatus);

                // Add export parameter
                params.append('export', 'csv');

                // Create download URL
                const exportUrl = '{{ route('task-ratings.index') }}?' + params.toString();

                // Create temporary link and trigger download
                const link = document.createElement('a');
                link.href = exportUrl;
                link.download = 'task-ratings-' + new Date().toISOString().split('T')[0] + '.csv';
            ...
            // Fix form submission to ensure proper filtering
